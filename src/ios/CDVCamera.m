...
+ (instancetype) createFromTakePictureArguments:(CDVInvokedUrlCommand*)command
{
    ...
    NSNumber* aspectX = [command argumentAtIndex:12 withDefault:nil];
    NSNumber* aspectY = [command argumentAtIndex:13 withDefault:nil];
    pictureOptions.aspectSize = CGSizeMake(1,1);
    if((aspectX != nil) &&(aspectY != nil)){
        pictureOptions.aspectSize = CGSizeMake([aspectX floatValue], [aspectY floatValue]);
    }
...
@interface CDVCamera ()<RSKImageCropViewControllerDataSource,RSKImageCropViewControllerDelegate>

@property (readwrite, assign) BOOL hasPendingOperation;
@property (nonatomic ,strong) NSDictionary *imageInfo;
@property (nonatomic ,strong) CDVCameraPicker *myPicker;
@property (nonatomic ,assign) CGSize aspectSize;
...
- (void)takePicture:(CDVInvokedUrlCommand*)command
{
    ...
    __strong CDVCamera* weakSelf = self;
    ...
    [self.commandDelegate runInBackground:^{
        ...
        _aspectSize = pictureOptions.aspectSize;
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    __strong CDVCameraPicker* cameraPicker = (CDVCameraPicker*)picker;
    _myPicker = cameraPicker;
    _imageInfo = info;	
    ...
    if ([mediaType isEqualToString:@"public.image"]) {
            ...
            [cameraPicker presentViewController:imageCropVC animated:YES completion:^{
        ...
        if (cameraPicker.pictureOptions.popoverSupported && (cameraPicker.pickerPopoverController != nil)) {
            ...
            [picker  dismissViewControllerAnimated:YES completion:invoke];
...

{
    ...
    [_imageInfo setValue:croppedImage forKey:UIImagePickerControllerOriginalImage];
    ...
    dispatch_block_t invoke = ^(void) {
        ...
        NSString* mediaType = [_imageInfo objectForKey:UIImagePickerControllerMediaType];
        if ([mediaType isEqualToString:(NSString*)kUTTypeImage]) {
            [self resultForImage:_myPicker.pictureOptions info:_imageInfo completion:^(CDVPluginResult* res) {
                [weakSelf.commandDelegate sendPluginResult:res callbackId:_myPicker.callbackId];
                weakSelf.hasPendingOperation = NO;
                weakSelf.pickerController = nil;
            }];
        }
        else {
            result = [self resultForVideo:_imageInfo];
            [weakSelf.commandDelegate sendPluginResult:result callbackId:_myPicker.callbackId];
            weakSelf.hasPendingOperation = NO;
            weakSelf.pickerController = nil;
        }
    };
    
    if (_myPicker.pictureOptions.popoverSupported && (_myPicker.pickerPopoverController != nil)) {
        [_myPicker.pickerPopoverController dismissPopoverAnimated:YES];
        _myPicker.pickerPopoverController.delegate = nil;
        _myPicker.pickerPopoverController = nil;
        invoke();
    } else {
        [[_myPicker presentingViewController] dismissViewControllerAnimated:YES completion:invoke];
...

{
    ...
    CGSize maskSize;
    CGFloat aspectHeight = _aspectSize.height/_aspectSize.width;
    CGFloat width=[[UIScreen mainScreen] bounds].size.width *0.9;
    
    if ([controller isPortraitInterfaceOrientation]) {
        
        maskSize = CGSizeMake(width, width * aspectHeight);
...

{
    ...
    //这里给出的是根据图片裁剪的尺寸来定义裁剪框形状的示例代码 官方给出的代码是定义一个三角形，这里只要利用UIBezierPath绘制一个闭合的路径再返回即可
